...
class GRU:
    def __init__(self, is_training, word_embeddings, settings):
        ...
        #这里判断是否是训练的主要目的就是：是否使用进度条即for i in tqdm.tqdm(range(big_num))，就这么点区别
        #big_num就是平常的batch，因为该过程比较耗时，训练集上面通常一个batch较少，不要看进度条，测试集通常就是整个数据集不分batch
        #所以要等很长时间，最好看一下进度
        if is_training:
            ...
            # sentence-level attention layer
            for i in tqdm.tqdm(range(big_num)):
                sen_repre.append(tf.tanh(attention_r[self.total_shape[i]:self.total_shape[i + 1]]))
                batch_size = self.total_shape[i + 1] - self.total_shape[i]
