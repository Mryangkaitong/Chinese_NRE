...
class re_framework:
    ...
    def __init__(self, train_data_loader, test_data_loader, max_length=60, batch_size=128):
        """
    ...
    def __test_bag__(self, model, ckpt=None, return_result=False):
        ...
        if self.test_data_loader.relfact_tot==0:
            ...
            F1 = (2 * np.array(prec) * np.array(recall) / (np.array(recall) + np.array(prec) + 1e-20)).max()
        print("\n[TEST] auc: {}".format(auc))
        print("\n[TEST] f1: {}".format(F1))
